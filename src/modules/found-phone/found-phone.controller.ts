import { FoundStatus, Prisma } from '@prisma/client';
...
export const createReport = async (
  ...
  const match = await prisma.lostPhone.findFirst({
    where: {
      imei,
      isDeleted: false,
    },
  ...
  const report = await prisma.foundPhone.create({
    data: {
      phoneModel,
      imei,
      brand,
      ...
      description,
      contactNumber,
      status: (status as FoundStatus) || 'FOUND',
      isMatched,
      userId,
    },
    include: {
  ...
  return sendResponse(
    ...
    201,
    true,
    'Found phone report created successfully',
    report
  );
};
...
export const updateReport = async (
  ...
  const existingReport = await prisma.foundPhone.findUnique({
    where: { id },
  });
  ...
  if (imei && imei !== existingReport.imei) {
    const newMatch = await prisma.lostPhone.findFirst({
  ...
  const updatedReport = await prisma.foundPhone.update({
    where: { id },
    data: {
      ...
      ...(status && { status: status as FoundStatus }),
      isMatched,
    },
  });
  ...
  return sendResponse(
    ...
    200,
    true,
    'Found phone report updated successfully',
    updatedReport
  );
};
...
export const deleteReport = async (
  ...
  return sendResponse(
    ...
    200,
    true,
    'Found phone report deleted successfully'
  );
};
...
export const getSingleReport = async (
  ...
  const report = await prisma.foundPhone.findUnique({
    where: { id },
    include: {
      user: {
        select: {
  ...
  return sendResponse(
    ...
    200,
    true,
    'Found phone report retrieved successfully',
    report
  );
};
...
export const getAllReports = async (
  ...
  if (status) {
    where.status = status as FoundStatus;
  }
  ...
  const [reports, total] = await Promise.all([
    ...
    prisma.foundPhone.count({ where }),
  ]);

  return res.status(200).json({
    success: true,
    message: 'Found phone reports retrieved successfully',
    data: reports,
    meta: {
      page: pageNum,
      limit: limitNum,
